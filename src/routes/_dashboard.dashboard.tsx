  ...
  return (
    <div className="px-6 py-5 space-y-5 w-full bg-beam-purple overflow-x-hidden">
      ...
      <div className="grid grid-cols-12 gap-5">
        ...
        {/* Recent Conversations — its own card below the chart */}
        <div className="col-span-8 bg-[#0B0B1A] border border-[#1C1C34] rounded-xl p-6">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-white font-semibold text-[15px] tracking-[-0.02em]">Recent Conversations</h3>
            <span className="text-[#7B5CFC] text-xs hover:text-[#9B84FF] cursor-pointer">View Inbox →</span>
          </div>
          <div className="space-y-1">
            {[
              { initials: "OA", name: "Omar Al Rashidi", grad: "from-[#7B5CFC]/40 to-[#00D4AA]/30", ChIcon: MessageCircle, time: "12m", msg: "I'd like to book a checkup for next week", status: "Qualified", sc: "#22C55E" },
              { initials: "SM", name: "Sara Mohammed", grad: "from-[#F59E0B]/40 to-[#FF4D6D]/30", ChIcon: MessageSquare, time: "34m", msg: "What are your teeth whitening prices?", status: "New", sc: "#7B5CFC" },
              { initials: "AK", name: "Ahmed Khalid", grad: "from-[#00D4AA]/40 to-[#7B5CFC]/30", ChIcon: Phone, time: "1h", msg: "Appointment confirmed for Thursday 2pm", status: "Contacted", sc: "#F59E0B" },
              { initials: "FR", name: "Fatima Rahman", grad: "from-[#FF4D6D]/40 to-[#7B5CFC]/30", ChIcon: MessageCircle, time: "2h", msg: "Can I reschedule my cleaning appointment?", status: "Qualified", sc: "#22C55E" },
            ].map((c) => {
              const ChIcon = c.ChIcon;
              return (
                <div key={c.name} className="flex items-center gap-3 py-2.5 rounded-lg hover:bg-white/[0.02] px-2 -mx-2 cursor-pointer transition-all">
                  <div className={`w-8 h-8 rounded-full bg-gradient-to-br ${c.grad} flex items-center justify-center text-white text-[10px] font-semibold flex-shrink-0`}>
                    {c.initials}
                  </div>
                  ...
                  <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
                    <span className="h-4 px-1.5 text-[10px] rounded-full inline-flex items-center font-medium" style={{ background: c.sc + "1A", color: c.sc }}>
                      {c.status}
                    </span>
                    <span className="bg-[#7B5CFC]/[0.12] text-[#9B84FF] text-[10px] px-1.5 rounded font-medium">AI</span>
                  </div>
                </div>
              );
            })}
        ...
        {/* Message Volume — its own card below */}
        <div className="col-span-8 bg-[#0B0B1A] border border-[#1C1C34] rounded-xl p-6">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-white font-semibold text-[15px] tracking-[-0.02em]">Message Volume — Last 7 Days</h3>
            <span className="text-[#4A4A6A] text-xs">Total: 3,855</span>
          </div>
              ...
              return (
                <div key={r.name} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-2 bg-[#1C1C34] rounded-full overflow-hidden">
                    <div className="h-full rounded-full transition-all" style={{ width: r.width, background: r.color }} />
                  </div>
                  <span className="text-white text-xs font-semibold w-12 text-right flex-shrink-0">{r.count}</span>
                </div>
              );
            })}
          </div>
